...
let
  ...
  readGitAttributesComment = path:
    ...
    if !fileExists then ""
      ...
      let
        ...
        commentLines = builtins.filter
          (line: lib.hasPrefix "# description: " line)
          lines;
      in
  ...
  maxTopLevelLen = foldl'
    (max: name: 
      let len = builtins.stringLength name + 1; # +1 for trailing slash
  ...
  maxSubLen = foldl'
    (max: len: if len > max then len else max)
    0
    (attrValues maxSubLengthPerDir);
  
  # Generate tree lines for each top-level directory
  generateSubTree = idx: name:
    let
      ...
      commentText =
        if gitAttrComment != "" then gitAttrComment
        else if name == "aspect" then "Aspect system core (aggregation, options)"
        else if name == "flake" then "Flake-level configuration (formatter, nix config, file generation)"
        else if name == "hm" then "Home Manager aspects and profiles"
        else if name == "sys" then "System-level configuration"
        else "";
      ...
      subLines = imap0
        (subIdx: subName:
          let
            ...
            aspectDesc = 
              if aspectDescriptions ? ${subName} then
                aspectDescriptions.${subName}
              else "";
            
            commentText = 
              if gitAttrComment != "" then gitAttrComment
              else if subName == "+profiles" then "Profile definitions and options"
              else if aspectDesc != "" then aspectDesc
              else "";
